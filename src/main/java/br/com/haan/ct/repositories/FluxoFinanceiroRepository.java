...
public interface FluxoFinanceiroRepository extends CrudRepository<FluxoFinanceiro,Long> {
    @Query(value="SELECT ((SELECT SUM(valor) FROM fluxoFinanceiro WHERE tipoFluxo = 1) - (SELECT SUM(valor) FROM fluxoFinanceiro WHERE tipoFluxo = 0)) AS valorTotal From fluxoFinanceiro where MONTH(dataPagamento) = :mes limit 1;" )
